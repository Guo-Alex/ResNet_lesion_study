...
__global__ void QuantTruncAbsForward(const int n, const int th, const float qstep, const Dtype* in,
		Dtype* out) {
...
void QuantTruncAbsLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
	...
	//qstep_ = this->layer_param_.quant_trunc_abs_param().qstep();
	...
	case QuantTruncAbsParameter_ProcessMethod_QUANTTRUNCABS:
		QuantTruncAbsForward<Dtype> <<<CAFFE_GET_BLOCKS(count),
				CAFFE_CUDA_NUM_THREADS>>>(count, threshold_, qstep_, bottom_data,
				top_data);
		break;
